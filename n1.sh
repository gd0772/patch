...
sed -i 's/"TTYD 终端"/"命令行"/g' `grep "TTYD 终端" -rl ./`
sed -i 's/"ShadowSocksR Plus+"/"SSR Plus+"/g' `grep "ShadowSocksR Plus+" -rl ./`
sed -i 's/"PassWall"/"Pass Wall"/g' `grep "PassWall" -rl ./`
...
sed -i 's/"Frps"/"Frp 服务端"/g' `grep "Frps" -rl ./`
sed -i 's/"Nps 内网穿透"/"Nps 客户端"/g' `grep "Nps 内网穿透" -rl ./`
sed -i 's/"解除网易云音乐播放限制"/"音乐解锁"/g' `grep "解除网易云音乐播放限制" -rl ./`
sed -i 's/"Docker CE 容器"/"Docker容器"/g' `grep "Docker CE 容器" -rl ./`
...
sed -i 's/"_("qBittorrent")"/"_("qB下载")"/g' `grep "BaiduPCS Web" -rl ./`
sed -i 's/"BaiduPCS Web"/"百毒网盘"/g' `grep "BaiduPCS Web" -rl ./`
sed -i 's/"IPSec VPN 服务器"/"IPSec 服务"/g' `grep "IPSec VPN 服务器" -rl ./`
sed -i 's/"PPTP VPN 服务器"/"PPTP 服务"/g' `grep "PPTP VPN 服务器" -rl ./`
sed -i 's/"V2ray 服务器"/"V2ray 服务"/g' `grep "V2ray 服务器" -rl ./`
sed -i 's/"SoftEther VPN 服务器"/"SoftEther"/g' `grep "SoftEther VPN 服务器" -rl ./`
sed -i 's/"OpenVPN 服务器"/"OpenVPN"/g' `grep "OpenVPN 服务器" -rl ./`
...
#echo
#TIME y "更新固件 编译日期"
#sed -i "s/2022.01.01/$(TZ=UTC-8 date "+%Y.%m.%d")/g" feeds/luci/modules/luci-mod-admin-full/luasrc/view/admin_status/index.htm
